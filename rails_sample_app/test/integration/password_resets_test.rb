...
class InvalidPasswordUpdateTest < PasswordResetForm
...
class ValidPasswordUpdateTest < PasswordResetForm
  ...
  test "update with valid " do
    assert is_logged_in?
    assert_not flash.empty?
    assert_redirected_to @reset_user
  end

  test "set digest nil" do
